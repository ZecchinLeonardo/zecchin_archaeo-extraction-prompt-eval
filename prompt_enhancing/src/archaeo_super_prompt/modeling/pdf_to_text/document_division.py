...
def get_page_ranges(
    doc_page_number: int, page_batch_size: int, border_page_nb: int | None=None
    ...
    def get_start_and_end_pages(border_page_nb: int):
        ...
        def flatten():
            for prange in split_into_batch_page_range(1, border_page_nb):
                yield prange
            for prange in split_into_batch_page_range(
