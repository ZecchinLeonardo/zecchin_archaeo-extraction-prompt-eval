...
class FindRitrovamento(dspy.Module):
    ...
    def forward(
        ...
        #         finding=cast(str, predicted_output.get("finding", UNIDENTIFIED_FINDING)),
        #     )
        # )
        # raw = predicted_output.get("ritrovamento", predicted_output.get("finding", UNIDENTIFIED_FINDING))
        return to_prediction(
            RitrovamentiOutputData(
                # finding=cast(str, raw),
                finding = cast(str, predicted_output.get("ritrovamento", UNIDENTIFIED_FINDING))
            )
        )
...
È stato rinvenuto un insediamento dell'età del bronzo con tracce materiali.""",
    ...
    def _select_answers(
        ...
        def to_ritrovamenti_data(row) -> RitrovamentiOutputData:
            """Try several possible attribute/column names on the answer row to
            retrieve the human-readable ritrovamento string.

            Return an empty finding string when none is available.
            """
            default_output = RitrovamentiOutputData(finding="")
            if row is None:
                return default_output

            # # candidate attribute/column names that may contain the label
            # candidates = [
            #     "university__iii_lev",
            #     "iii_livello",
            #     "iii_lev",
            ...
            #     if val is not None:
            #         # some sources store JSON-like or dict-like strings; coerce to str
            #         try:
            #             s = str(val)
            #         except Exception:
            #             s = ""
            #         return RitrovamentiOutputData(finding=s)

            val = getattr(row, "university__iii_lev", None)
            if val is not None:
                try:
                    s = str(val)
                except Exception:
